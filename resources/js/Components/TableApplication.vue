...
const approve = () => {
    ...
    if (props.title === "For Interview" && !form.notes) {
        form.setError('notes', 'Notes is required.');
    } else {
        form.put(
...
<template>
    <Transition enter-active-class="transition ease-in-out duration-300"
        ...
        <Teleport to="body">
            <div v-if="approveModalVisibility" @click="hideApproveModal"
                class="bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-30 transition duration-200"></div>

            <div v-else-if="disapproveModalVisibility" @click="hideDisapproveModal"
                class="bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-30 transition duration-200"></div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            <tr v-for="(role, index) in roles.data" :key="role.id" @click="showInfoModal(roles.data[index])"
                                ...
                                <td class="px-2 py-4 space-x-2 whitespace-nowrap">
                                    ...
                                    <button @click="showApproveModal(role.id)" type="button" id="approveUserButton"
    ...
    <Transition enter-from-class="translate-x-full" enter-active-class="transition-transform translate-x-0"
        ...
        <div v-if="viewInfoModalVisibility" id="drawer-delete-product-default"
            ...
            <div class="flex flex-col gap-4 justify-center w-full py-4">
                <button @click="showApproveModal(currentUpdatingUserID)"
    ...
    <Transition enter-from-class="translate-x-full" enter-active-class="transition-transform translate-x-0"
        leave-active-class="transition-transform translate-x-0" leave-to-class="translate-x-full">
        <div v-if="approveModalVisibility" id="drawer-delete-product-default"
            class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto bg-white dark:bg-gray-800"
            tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
            <h5 id="drawer-label"
                class="inline-flex items-center text-sm font-semibold text-gray-500 uppercase dark:text-gray-400">
            ...
            <button @click="hideApproveModal" type="button" aria-controls="drawer-delete-product-default"
                class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                    xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd"
            ...
            <h3 v-if="props.title !== 'For Interview'" class="mb-6 text-lg text-gray-500 dark:text-gray-400">
                Are you sure you want to approve this {{ title }}?
            </h3>
            <h3 v-else class="mb-6 text-lg text-gray-500 dark:text-gray-400">
                Before approving this application, please add a note so that the applicant will know what documents are
                needed to submit.
            </h3>
            <TextArea v-if="props.title === 'For Interview'" id="notes" v-model="form.notes" rows="3" label="Notes"
                placeholder="Notes" :error="form.errors.notes" />
            <form @submit.prevent="approve" class="inline-block mt-6">
                <button v-if="props.title !== 'For Interview'" type="submit"
        ...
        <div v-if="updateModalVisibility" id="drawer-update-product-default"
            ...
            <form @submit.prevent="update">
                <div class="space-y-10">
                    ...
                    <div>
                        <TextArea v-if="props.title === 'In Progress'" id="notes" v-model="form.notes" rows="3"
                            label="Notes" placeholder="Notes" :error="form.errors.notes" />
                    </div>
                </div>
